...
function prepare(adapter, key, job) {
  ...
  const body = normalizeInput({ model: request.model, messages: request.messages, maxTokens: request.maxTokens, temperature: request.temperature, thinking: request.thinking, stream: request.stream })
  ...
  return {
    ...
    init: { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'text/event-stream, application/json' }, body: JSON.stringify(body), signal: null },
  }
}
...
function responseContent(format, response) {
  ...
  if (format === 'ollama') return { content: response.message?.content || response.response || '', reasoning: '' }
  if (format === 'gemini') {
