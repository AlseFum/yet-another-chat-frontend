...
export function createRetrier(maxRetries = 2) {
  return ({ messages, output, errors, attempt }) => {
    ...
    return [
      ...
      { role: 'user', content: `审核你刚才的回答并修复校验错误：${errors.join('；')}。只返回修复后的结果。` },
    ]
  }
}
...
export class JobRequest {
  constructor({ messages = [], model, maxTokens = 4096, temperature = 0.7, thinking = null, validator = null, retrier = null, stream = true } = {}) {
    this.messages = Array.isArray(messages) ? messages : []
    this.model = model
    this.maxTokens = Number(maxTokens) || 4096
    this.temperature = Number.isFinite(Number(temperature)) ? Number(temperature) : 0.7
    this.thinking = thinking === true ? { type: 'enabled' } : thinking?.type === 'enabled' ? { type: 'enabled' } : null
