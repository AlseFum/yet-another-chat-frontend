...
const props = defineProps({ applications: Object, customSettings: Object, updateSetting: Function })
const emit = defineEmits(['notify'])
const selectedId = ref('')

const entries = computed(() => [...(props.applications?.values() || [])].filter(application => Object.keys(application.constructor.schema?.() || {}).length))
...
<template>
  <section class="custom-settings-view view">
    <aside class="custom-settings-list">
      <header><p class="eyebrow">WORKSPACE</p><h2>应用设置</h2></header>
      <button v-for="application in entries" :key="application.id" :class="{ active: selected?.id === application.id }" @click="selectedId = application.id">
        <span>{{ application.id }}</span><span>{{ Object.keys(application.constructor.schema()).length }}</span>
      </button>
      <p v-if="!entries.length" class="empty-state">暂无可配置的 Application</p>
    </aside>
    <main v-if="selected" class="custom-settings-editor">
      <header><div><p class="eyebrow">{{ selected.id.toUpperCase() }}</p><h1>应用设置</h1></div><UiButton variant="primary" disabled>已自动保存</UiButton></header>
      <div class="custom-settings-fields">
        <label v-for="(field, name) in schema" :key="name" class="custom-setting-field">
          <span class="custom-setting-field__label">{{ field.label || name }}<small v-if="field.description">{{ field.description }}</small></span>
          <UiCombobox v-if="field.type === 'select'" :model-value="values[name]" :options="field.options || []" :title="field.label || name" @update:model-value="update(name, $event)" />
