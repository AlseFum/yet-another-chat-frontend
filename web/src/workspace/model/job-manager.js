  ...
  apply(snapshot, event = null) {
    ...
    const enrichedEvent =
      event?.type === "result"
        ? {
            ...event,
            value: event.value ?? this.value,
            rawText: event.rawText ?? this.responseText,
            toolCalls: event.toolCalls ?? this.toolCalls ?? [],
          }
        : event?.type === "delta"
          ? {
              ...event,
              responseText: this.responseText,
              responseReasoning: this.reasoning,
            }
          : event;
    this.events.next(
      enrichedEvent || { jobId: this.id, type: "state", state: this.status },
    );
  }
...
export class BrowserJobManager {
  constructor(transport) {
    ...
    this.subscription = transport.onEvent((message) => {
      if (message.type === "transport.error")
        return this.events.next({ source: "transport", event: message });
  ...
  list(ids = null) {
    return (ids || [...this.jobs.keys()])
      .map((id) => this.get(id))
      .filter(Boolean);
  }
  source(jobId) {
    return this.sources.get(jobId) || "server";
  }
  snapshot(job) {
    return {
      ...job.toJSON(),
      mode: this.source(job.id),
      metadata: this.metadata.get(job.id) || {},
    };
  }
  snapshots(ids = null) {
    return this.list(ids).map((job) => this.snapshot(job));
  }

  create({ request, keyRef, metadata = {} }) {
    if (!(keyRef instanceof KeyRef)) throw new TypeError("Job 需要 KeyRef");
    const normalizedRequest =
      request instanceof JobRequest ? request : new JobRequest(request);
    if (keyRef.type === "temporary") {
      ...
      this.jobSubscriptions.set(
        job.id,
        job.onEvent((event) =>
          this.events.next({ source: "direct", job, event }),
        ),
      );
      job.result.catch(() => {});
      return job;
    }
    const job = new BrowserRemoteJob({
      id: nanoid(),
      keyId: keyRef.keyId,
    ...
    this.events.next({
      source: "local",
      job,
      event: { jobId: job.id, type: "state", state: "idle" },
    });
    return job;
  }
  ...
  async start(jobId) {
    ...
    const snapshot = await this.transport.launchJob({
      jobId: job.id,
      keyId: job.keyId,
      request: job.request,
    });
  ...
  async load(ids, options = {}) {
    ...
    const { jobs = [], missingIds = [] } = await this.transport.loadJobs(
      ids,
      options,
    );
    ...
    for (const id of missingIds) {
      ...
      this.jobs.set(
        id,
        new BrowserRemoteJob({ id, status: "missing", error: "Job 不存在" }),
      );
  ...
  async loadDetail(jobId) {
    const { jobs = [] } = await this.transport.loadJobs([jobId], {
      detail: true,
    });
  ...
  async clean(jobId) {
    ...
    if (!terminalStates.has(job.status))
      throw new Error("Job 仍在运行，不能清理");
    if (this.source(jobId) === "server" && job.status !== "missing")
      await this.transport.cleanJob(jobId);
