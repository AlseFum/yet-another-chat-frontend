...
import {
  expandTextReferences,
  formatTextResource,
} from "../../workspace/text-reference.js";
...
export async function compileWorldContext(worldContext, workspace) {
  const parts = [
    await expandTextReferences(worldContext?.content || "", workspace),
  ];
  for (const textId of worldContext?.textResourceIds || []) {
    const result = workspace.resources.borrow("text", textId);
    if (!result.ok) throw result.error;
    const lease = result.value;
    try {
      const text = lease.read();
      parts.push(formatTextResource(text, textId));
    } finally {
      lease.release();
    }
  }
