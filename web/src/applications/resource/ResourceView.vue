...
import { formatPersonaSectionName, parsePersonaSectionName, personaItemReference, personaSectionScopes, validatePersona } from './persona-resource.js'
...
const personaScopeIcons = { all: 'scope-all', chat: 'scope-chat', 'talk:private': 'scope-talk', 'talk:public': 'scope-public' }
let saveTimer = null
const labels = { text: ['文本', 'file'], preset: ['预设', 'preset'], tool: ['工具', 'tool'], persona: ['Persona', 'message'] }
...
const personaIssues = computed(() => type.value === 'persona' && selected.value
  ? validatePersona(selected.value, new Set(textResources.value.map(item => item.id)))
  : [])
const personaStats = computed(() => {
  const sections = selected.value?.sections || []
  const itemCount = sections.reduce((total, section) => total + Math.max(0, section.length - 1), 0)
  const referenceCount = sections.reduce((total, section) => total + section.slice(1).filter(item => personaItemReference(item) !== null).length, 0)
  return { sections: sections.length, items: itemCount, references: referenceCount }
})
...
function personaScopeLabel(scope) {
  return personaSectionScopes.find(option => option.value === scope)?.label || '未知作用域'
}

function personaSectionIssue(sectionIndex) {
  return personaIssues.value.some(issue => issue.sectionIndex === sectionIndex)
}

function personaItemIssue(sectionIndex, itemIndex) {
  return personaIssues.value.some(issue => issue.sectionIndex === sectionIndex && issue.itemIndex === itemIndex)
}

function personaItemKey(section, itemIndex) {
  const item = section[itemIndex + 1]
  const occurrence = section.slice(1, itemIndex + 1).filter(value => value === item).length
  return `${item}:${occurrence}`
}

function setSectionScope(index, scope) {
  const section = personaSection(index)
  selected.value.sections[index][0] = formatPersonaSectionName(scope, section.title)
}

function setSectionTitle(index, title) {
  const section = personaSection(index)
  selected.value.sections[index][0] = formatPersonaSectionName(section.scope, title)
}
...
function addPersonaAction() {
  ...
  selected.value.orchestrator.actions.push({ id: `action-${selected.value.orchestrator.actions.length + 1}`, name: '新行动', description: '', triggers: [], tools: [], inputSchema: { type: 'object', properties: {}, additionalProperties: false }, outputSchema: { type: 'object', properties: {}, additionalProperties: false }, sideEffects: 'none' })
}

function removePersonaAction(index) { selected.value.orchestrator.actions.splice(index, 1) }
function schemaText(schema) { return JSON.stringify(schema, null, 2) }
function setActionSchema(action, key, value) {
  try { action[key] = JSON.parse(value) } catch { /* Keep the last valid schema while editing. */ }
}
function triggerText(action) { return (action.triggers || []).join('\n') }
function setActionTriggers(action, value) { action.triggers = value.split('\n').map(item => item.trim()).filter(Boolean) }
function toolText(action) { return (action.tools || []).join('\n') }
function setActionTools(action, value) { action.tools = value.split('\n').map(item => item.trim()).filter(Boolean) }
...
function startPersonaItemDrag(event, sectionIndex, itemIndex) {
  ...
  personaDropTarget.value = null
  event.dataTransfer.effectAllowed = 'move'
  event.dataTransfer.setData('text/plain', `persona-item:${sectionIndex}:${itemIndex}`)
}
...
function setPersonaDropTarget(event, type, sectionIndex, itemIndex = null) {
  ...
  const after = event.currentTarget.getBoundingClientRect().top + event.currentTarget.offsetHeight / 2 < event.clientY
...
function dropPersonaItem(event, sectionIndex, itemIndex = null) {
  ...
  let target = itemIndex === null ? targetSection.length : itemIndex + 1 + (personaDropTarget.value?.after ? 1 : 0)
...
function startPersonaPointerDrag(event, type, sectionIndex, itemIndex = null) {
  ...
  personaDrag.value = { type, sectionIndex, itemIndex, pointerId: event.pointerId }
  personaDropTarget.value = null
  document.addEventListener('pointermove', handlePersonaPointerMove, { passive: false })
  document.addEventListener('pointerup', handlePersonaPointerUp, { once: true })
  document.addEventListener('pointercancel', handlePersonaPointerUp, { once: true })
}
...
function handlePersonaPointerMove(event) {
  ...
  const item = target.closest('[data-persona-item-section-index][data-persona-item-index]')
  if (item) {
  ...
  if (end) personaDropTarget.value = { type: 'item', sectionIndex: Number(end.dataset.personaItemEndSectionIndex), itemIndex: null, after: true }
}
...
function handlePersonaPointerUp(event) {
  ...
  if (target?.type === 'section' && drag.type === 'section') {
    ...
    reorderPersonaItem(drag.sectionIndex, drag.itemIndex, target.sectionIndex, target.itemIndex, target.after)
  }
...
function reorderPersonaSection(sourceIndex, targetIndex, after = false) {
  ...
  selected.value.sections.splice(Math.max(0, Math.min(target, selected.value.sections.length)), 0, section)
}

function reorderPersonaItem(sourceSectionIndex, sourceItemIndex, targetSectionIndex, targetItemIndex = null, after = false) {
  ...
  let target = targetItemIndex === null ? targetSection.length : targetItemIndex + 1 + (after ? 1 : 0)
  if (sourceSection === targetSection && sourceItemIndex + 1 < target) target--
  targetSection.splice(Math.max(1, Math.min(target, targetSection.length)), 0, item)
}
...
function setPersonaItemMode(section, index, mode) {
  const source = index + 1
  if (mode === 'reference') section[source] = textResources.value[0] ? `@${textResources.value[0].id}` : '@'
...
<template>
    ...
    <main v-if="selected && (!mobile || selectedId)" class="resource-editor">
      ...
      <div v-if="type === 'tool'" class="tool-code-editor">
       ...
       <div v-else-if="type === 'persona'" class="persona-editor">
          <div class="persona-editor__intro">
            <div class="persona-editor__intro-copy"><p class="eyebrow">PERSONA BLUEPRINT</p><strong>Prompt Sections</strong><span>把角色拆成有顺序的主题模块，再按 Chat / Talk 场景投影。</span></div>
            <div class="persona-editor__intro-actions"><div class="persona-editor__stats"><span><strong>{{ personaStats.sections }}</strong> sections</span><span><strong>{{ personaStats.items }}</strong> items</span><span><strong>{{ personaStats.references }}</strong> refs</span></div><UiButton size="sm" @click="addPersonaSection"><AppIcon name="plus" />添加 Section</UiButton></div>
          </div>
          <div class="persona-editor__tip"><AppIcon name="info" size="14" /><span>建议按“身份、目标、原则、能力、交流方式”组织内容。拖动左侧手柄可调整顺序，`@Text` 会在运行时展开为引用文本。</span></div>
          <TransitionGroup name="persona-section" tag="div" class="persona-section-list">
          <div
            v-for="(section, sectionIndex) in (selected.sections || [])"
            :key="section"
            class="persona-section-card"
            :data-persona-section-index="sectionIndex"
            :class="{ 'has-issue': personaSectionIssue(sectionIndex), 'is-dragging': personaDrag?.type === 'section' && personaDrag.sectionIndex === sectionIndex, 'is-drop-before': personaDropTarget?.type === 'section' && personaDropTarget.sectionIndex === sectionIndex && !personaDropTarget.after, 'is-drop-after': personaDropTarget?.type === 'section' && personaDropTarget.sectionIndex === sectionIndex && personaDropTarget.after }"
            @dragover="setPersonaDropTarget($event, 'section', sectionIndex)"
            @drop="dropPersonaSection($event, sectionIndex)"
          >
           <header>
              <span class="persona-drag-handle" draggable="true" title="拖动 Section 排序" @pointerdown="startPersonaPointerDrag($event, 'section', sectionIndex)" @dragstart="startPersonaSectionDrag($event, sectionIndex)" @dragend="endPersonaDrag"><AppIcon name="menu" size="14" /></span>
              <span class="persona-section-card__order">{{ String(sectionIndex + 1).padStart(2, '0') }}</span>
              <div class="persona-section-heading">
                <input class="field persona-section-title" :value="personaSection(sectionIndex).title" placeholder="Section 标题，例如：身份与职责" @input="setSectionTitle(sectionIndex, $event.target.value)" />
                <div class="persona-scope-picker" role="radiogroup" :aria-label="`Section ${sectionIndex + 1} 作用域`">
                  <button v-for="scope in personaSectionScopes" :key="scope.value" type="button" class="persona-scope-option" :class="{ active: personaSection(sectionIndex).scope === scope.value }" :aria-label="scope.label" :title="scope.label" role="radio" :aria-checked="personaSection(sectionIndex).scope === scope.value" @click="setSectionScope(sectionIndex, scope.value)"><AppIcon :name="personaScopeIcons[scope.value]" size="14" /></button>
                </div>
              </div>
             <div class="persona-row-actions">
               <UiButton variant="ghost" size="icon" title="上移 Section" :disabled="sectionIndex === 0" @click="movePersonaSection(sectionIndex, -1)"><AppIcon name="back" size="13" /></UiButton>
               <UiButton variant="ghost" size="icon" title="下移 Section" :disabled="sectionIndex === selected.sections.length - 1" @click="movePersonaSection(sectionIndex, 1)"><AppIcon name="back" size="13" /></UiButton>
               <UiButton variant="ghost" size="icon" title="删除 Section" @click="removePersonaSection(sectionIndex)"><AppIcon name="trash" size="13" /></UiButton>
             </div>
            </header>
            <div class="persona-section-card__meta"><span>{{ personaScopeLabel(personaSection(sectionIndex).scope) }}</span><span>{{ Math.max(0, section.length - 1) }} 个内容项</span><span v-if="personaSectionIssue(sectionIndex)" class="is-error"><AppIcon name="info" size="11" />需要检查</span></div>
            <TransitionGroup name="persona-item" tag="div" class="persona-section-items">
             <div
               v-for="(item, itemIndex) in section.slice(1)"
                ...
                :class="{ 'has-issue': personaItemIssue(sectionIndex, itemIndex), 'is-dragging': personaDrag?.type === 'item' && personaDrag.sectionIndex === sectionIndex && personaDrag.itemIndex === itemIndex, 'is-drop-before': personaDropTarget?.type === 'item' && personaDropTarget.sectionIndex === sectionIndex && personaDropTarget.itemIndex === itemIndex && !personaDropTarget.after, 'is-drop-after': personaDropTarget?.type === 'item' && personaDropTarget.sectionIndex === sectionIndex && personaDropTarget.itemIndex === itemIndex && personaDropTarget.after }"
               @dragover="setPersonaDropTarget($event, 'item', sectionIndex, itemIndex)"
               @drop="dropPersonaItem($event, sectionIndex, itemIndex)"
             >
                <span class="persona-drag-handle persona-item__drag" draggable="true" title="拖动 Item 排序" @pointerdown="startPersonaPointerDrag($event, 'item', sectionIndex, itemIndex)" @dragstart.stop="startPersonaItemDrag($event, sectionIndex, itemIndex)" @dragend="endPersonaDrag"><AppIcon name="menu" size="13" /></span>
                <div class="persona-item-type-picker" role="radiogroup" :aria-label="`Item ${itemIndex + 1} 类型`">
                  <button type="button" class="persona-item-type-option" :class="{ active: personaItemReference(item) === null }" aria-label="内联文本" title="内联文本" role="radio" :aria-checked="personaItemReference(item) === null" @click="setPersonaItemMode(section, itemIndex, 'inline')"><AppIcon name="file" size="13" /></button>
                  <button type="button" class="persona-item-type-option" :class="{ active: personaItemReference(item) !== null }" aria-label="Text Resource 引用" title="Text Resource 引用" role="radio" :aria-checked="personaItemReference(item) !== null" @click="setPersonaItemMode(section, itemIndex, 'reference')"><AppIcon name="at" size="13" /></button>
                </div>
               <select v-if="personaItemReference(item) !== null" class="field persona-item__content" :value="personaItemReference(item)" @change="setPersonaReference(section, itemIndex, $event.target.value)">
                 <option value="" disabled>{{ textResources.length ? '选择 Text Resource' : '暂无 Text Resource' }}</option>
                 <option v-if="personaItemReference(item) && !textResources.some(text => text.id === personaItemReference(item))" :value="personaItemReference(item)">缺失 · {{ personaItemReference(item) }}</option>
                 <option v-for="text in textResources" :key="text.id" :value="text.id">{{ text.name }} · {{ text.id }}</option>
               </select>
                <textarea v-else v-model="section[itemIndex + 1]" class="field persona-item__content" rows="3" placeholder="输入这一项的 Prompt 文本；同一主题的相关句子放在同一个 Section 中" />
               <div class="persona-row-actions">
                 <UiButton variant="ghost" size="icon" title="上移 Item" :disabled="itemIndex === 0" @click="movePersonaItem(section, itemIndex, -1)"><AppIcon name="back" size="12" /></UiButton>
                 <UiButton variant="ghost" size="icon" title="下移 Item" :disabled="itemIndex === section.length - 2" @click="movePersonaItem(section, itemIndex, 1)"><AppIcon name="back" size="12" /></UiButton>
                 <UiButton variant="ghost" size="icon" title="删除 Item" @click="removePersonaItem(section, itemIndex)"><AppIcon name="trash" size="12" /></UiButton>
               </div>
             </div>
              <div class="persona-item-drop-end" :data-persona-item-end-section-index="sectionIndex" :class="{ active: personaDropTarget?.type === 'item' && personaDropTarget.sectionIndex === sectionIndex && personaDropTarget.itemIndex === null }" @dragover="setPersonaDropTarget($event, 'item', sectionIndex)" @drop="dropPersonaItem($event, sectionIndex)">
                <UiButton class="persona-add-item" variant="ghost" size="sm" @click="addPersonaItem(section)"><AppIcon name="plus" />添加文本或引用</UiButton>
              </div>
            </TransitionGroup>
          </div>
          </TransitionGroup>
         <p v-if="!selected.sections?.length" class="empty-state">暂无 Section，请先添加一个。</p>
         <section class="persona-actions-editor">
           <header><div><p class="eyebrow">ACTION CONTRACTS</p><h3>交给 Orchestrator 的行动契约</h3><span>Orchestrator 依据这些 Schema 决定实例能否行动，以及应该提供哪些 Context。</span></div><UiButton size="sm" @click="addPersonaAction"><AppIcon name="plus" />添加 Action</UiButton></header>
           <label>能力摘要<textarea v-model="selected.orchestrator.summary" class="field" rows="3" placeholder="简要说明该 Persona 适合处理什么问题" /></label>
           <article v-for="(action, actionIndex) in selected.orchestrator.actions" :key="action.id" class="persona-action-card">
             <header><span>{{ String(actionIndex + 1).padStart(2, '0') }}</span><input v-model="action.id" class="field" placeholder="action-id" /><input v-model="action.name" class="field" placeholder="行动名称" /><select v-model="action.sideEffects" class="field"><option value="none">无副作用</option><option value="workspace">修改 Workspace</option></select><UiButton variant="ghost" size="icon" title="删除 Action" @click="removePersonaAction(actionIndex)"><AppIcon name="trash" size="13" /></UiButton></header>
             <label>说明<textarea v-model="action.description" class="field" rows="2" /></label>
             <div class="persona-action-schemas"><label>适用场景（每行一条）<textarea class="field" rows="3" :value="triggerText(action)" @input="setActionTriggers(action, $event.target.value)" /></label><label>允许的 Tool ID（每行一条）<textarea class="field" rows="3" :value="toolText(action)" @input="setActionTools(action, $event.target.value)" /></label></div>
             <div class="persona-action-schemas"><label>Input Schema<textarea class="field" rows="10" :value="schemaText(action.inputSchema)" @change="setActionSchema(action, 'inputSchema', $event.target.value)" /></label><label>Output Schema<textarea class="field" rows="10" :value="schemaText(action.outputSchema)" @change="setActionSchema(action, 'outputSchema', $event.target.value)" /></label></div>
           </article>
           <p v-if="!selected.orchestrator.actions.length" class="empty-state">暂无 Action，Orchestrator 不会调度此 Persona。</p>
         </section>
         <div v-if="personaIssues.length" class="persona-issues"><strong>需要处理</strong><span v-for="(issue, index) in personaIssues" :key="index">{{ issue.message }}</span></div>
       </div>
    ...
    <UiModal v-model="personaGenerateOpen" title="生成 Persona" description="描述你希望创建的角色、能力、语气和场景。现有 Sections 会作为上下文提供给模型。" size="md" :close-on-backdrop="!personaGenerating">
      <label class="persona-generate-prompt">本次生成需求<textarea v-model="personaGeneratePrompt" class="field" rows="8" placeholder="例如：创建一名严谨的系统架构师。Chat 中直接帮助用户澄清需求；Talk 中负责识别方案边界、技术风险和参与者观点之间的冲突。" :disabled="personaGenerating" @keydown.ctrl.enter="generatePersona" /></label>
      <p v-if="personaGenerateError" class="persona-generate-error">{{ personaGenerateError }}</p><p class="persona-generate-hint">此内容仅用于本次生成，不会保存到 Persona 或 Custom Settings。可按 Ctrl + Enter 提交。</p>
      <template #footer="{ close }"><UiButton variant="ghost" :disabled="personaGenerating" @click="close">取消</UiButton><UiButton variant="primary" :disabled="!personaGeneratePrompt.trim() || personaGenerating" @click="generatePersona"><AppIcon name="robot" />{{ personaGenerating ? '生成中' : '开始生成' }}</UiButton></template>
    </UiModal>
