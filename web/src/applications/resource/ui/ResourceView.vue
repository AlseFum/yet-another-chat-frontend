...
const props = defineProps({ type: String, resources: Object })
...
const language = computed(() => props.type === 'tool' ? 'javascript' : 'markdown')

watch(() => props.type, () => { selectedId.value = '' })

function add() {
  const item = { id: `fixture-${Date.now()}`, name: `新${labels[props.type][0]}`, content: '', description: '' }
...
<template>
  <section class="resource-view view">
    ...
    <main v-if="selected" class="resource-editor">
      <header><div><p class="eyebrow">{{ type.toUpperCase() }}</p><h1>编辑{{ labels[type][0] }}</h1></div><div><UiButton><AppIcon name="robot" />AI 生成</UiButton><UiButton variant="primary"><AppIcon name="check" />保存</UiButton></div></header>
      ...
      <footer><span>fixture only · changes reset on reload</span><UiButton variant="danger"><AppIcon name="trash" />删除</UiButton></footer>
    </main>
  </section>
</template>
